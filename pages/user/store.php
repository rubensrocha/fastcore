    ...
    if ($db->NumRows() > 0) {
        ...
        if ($profit > $cnf['min_s']) {
            ...
            if ($time <= $pers['end']) {
                ...
                echo '<div class="alert alert-danger">Срок персонажа истек!</div>';
            }
        } else {
            echo '<div class="alert alert-danger">Минимальная сумма для сбора ' . $cnf['min_s'] . ' руб.</div>';
        }
    } else {
        echo '<div class="alert alert-danger">Нечего собирать! </div>';
    }
}
...
if ($db->NumRows() > 0) {
                    ...
                    <div class="p-2"><img src="/img/items/<?= $pers['tarif'] ?>.png" style="max-width: 50%;">
                        ...
                        if (time() <= $endlife2) {
                            ...
                            <div class="badge badge-light mb-1 mt-2" style="font-size: 110%;display: block;">
                                <span
                                    class="text-info"><?= $profit = $func->SumCalc($pers['speed'], $pers['tarif'], $pers['last']) ?></span>
                                <br/>
                                <small>Доход:</small>
                            </div>

                            <span class="badge badge-light mb-1" style="font-size: 95%;display: block;"
                                  title="Осталось дней до завершения персонажа">
                            ...
                            echo '<div class="alert alert-danger mt-2 mb-0">Завершен</div>';
                        } ?>
                    </div>
                </div>
            </div>
    ...
    echo '<div class="alert alert-danger text-center">У Вас нет персонажей, купите их!</div>';
}
