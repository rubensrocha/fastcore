...
if (isset($_POST["change"])) {
    ...
    if ($sum >= $min_sum) {
        if ($user['money_p'] >= $sum) {
            ...
            echo '<center class="alert alert-warning">Недостаточно средств</center>';
        }
    } else {
        echo '<center class="alert alert-danger">Минимум ' . $min_sum . ' руб.</center>';
    }
}
?>

<h5 class="text-center">Обменяйте баланс с вывода на баланс для покупок и получите <b>+<?= $cnf['p_swap'] ?>%</b> при
    обмене.</h5>
<div class="row">
        ...
        <div class="text-center">
            <form action="" method="post">
                <div class="input-group mb-2">
                    ...
                    <input type="text" class="form-control" id="change" name="change" onkeyup="GetSumPer();"
                           value="10"/>
                    <div class="input-group-append"><span class="input-group-text" title="Получите на счет для покупок"><small>+</small> <span
                                id="res_sum" name="res">10.5</span></span></div>
                </div>
            ...
            <script>
                ...
                function GetSumPer() {
                    let sum = parseFloat(document.getElementById("change").value);
