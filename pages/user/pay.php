                                        ...
                                        if ($payeer->isAuth()) {
                                            ...
                                            if ($arBalance["auth_error"] == 0) {
                                                ...
                                                if ($balance >= $sum) {
                                                    ...
                                                    echo '<div class="alert alert-danger">Ошибка 629. Сообщите о ней администратору!</div>';
                                                }
                                            } else {
                                                echo '<div class="alert alert-danger">Ошибка 630. Не удалось выплатить! Попробуйте позже</div>';
                                            }
                                        } else {
                                            echo '<div class="alert alert-danger">Ошибка 631. Не удалось выплатить! Попробуйте позже</div>';
                                        }
                                    } else {
                                        echo '<div class="alert alert-warning">Вы указали больше, чем имеется на вашем счету</div>';
                                    }
                                } else {
                                    echo '<div class="alert alert-danger">Пополните баланс минимум на ' . $accPay . ' руб! После этого выплаты будут доступны.</div>';
                                }
                            } else {
                                echo '<div class="alert alert-warning">Минимальная сумма для выплаты составляет ' . $minPay . ' руб!</div>';
                            }
                        } else {
                            echo '<div class="alert alert-warning">Максимальная сумма для выплаты составляет ' . $maxPay . ' руб!</div>';
                        }
                    } else {
                        echo '<div class="alert alert-danger">Вы уже заказывали выплату за последние ' . $todayLimit . ' час(ов).</div>';
                    }
                } else {
                    echo '<div class="alert alert-warning">Номер счета ' . $purse . ' указан неверно</div>';
                }
            } else {
                echo '<div class="alert alert-warning">Сохраните кошелек ' . $pyName . ' в настройках.</div>';
            }
        }
        ?>
    </center>
                    ...
                    <form action="" method="POST">
                        <div class="p-3">
                            ...
                            <div class="input-group">
                                <div class="input-group-prepend"><span class="input-group-text"><?= $pyName ?></span>
                                </div>
                                <input class="form-control" type="text" placeholder="Пример: <?= $varPy ?>"
                                       value="<?= ((!empty($valid)) > 0) ? $valid : FALSE ?>" name="purse">
                            </div>
                        </div>
                        <hr class="my-0">
                        <div class="p-3"><label>Сумма выплаты</label>
                            <div class="input-group">
                                ...
                                <input class="form-control" placeholder="Сумма выплаты" name="sum"
                                       value="<?= round($user['money_p'], 2) ?>">
                                <div class="input-group-append">
                                    <button class="btn btn-success" name="pay" type="submit">Вывести</button>
                                </div>
                            </div>
                        </div>
                    </form>
...
<div class="card mb-2">
    ...
    <div class="row m-1">
        <div class="col-md-4 p-1">
            <a href="/user/pay/payeer" class="card p-5 bg-light mb-0"
               style="background: url(/img/pay/ps/1.png) no-repeat center center;background-size: 120px;"><br/><br/><br/></a>
        </div>
        <div class="col-md-4 p-1">
            <a href="/user/pay/qiwi" class="card p-5 bg-light mb-0"
               style="background: url(/img/pay/ps/2.png) no-repeat center center;background-size: 120px;"><br/><br/><br/></a>
        </div>
        <div class="col-md-4 p-1">
            <a href="/user/pay/yandex" class="card p-5 bg-light mb-0"
               style="background: url(/img/pay/ps/3.png) no-repeat center center;background-size: 120px;"><br/><br/><br/></a>
        </div>
    </div>
</div>