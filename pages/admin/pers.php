...
if (isset($pg->segment[2]) && !empty($pg->segment[2] === 'edit')) {
    $type = 2;
} // выплаты
?>
<h3>Персонажи</h3>
...
# Добавление персонажа
        ...
        <div class="col-lg-6">
            <div class="card">
                ...
                <form action="" method="post" class="card-body">
                    ...
                    <input type="text" name="img" placeholder="Картинка пример: число-1 /items/1.png"
    ...
    if (isset($pg->segment[2])) {
        ...
        if ($db->NumRows() != 1) {
            echo '<div class="alert alert-danger">Персонаж с таким ID не найдена</div>';
            return;
        }
            ...
            <div class="card-body">
                <form action="" method="post" class="m-0">
                    ...
                    <input type="text" name="title" class="form-control mb-2"
                           placeholder="Например: Уровень-<?= $pers['id'] ?>" value="<?= $pers['title'] ?>"/>
                    <b>Доходность в час (РУБ.):</b>
                    <input name="speed" class="form-control mb-2" placeholder="Например: 0.05"
                           value="<?= $pers['speed'] ?>"/>
                    <b>Цена РУБ:</b>
                    <input name="price" class="form-control mb-2" placeholder="Например: 100"
                           value="<?= $pers['price'] ?>"/>
                    <b>Срок:</b>
                    <input name="period" class="form-control mb-2" placeholder="Например: 100"
                           value="<?= $pers['period'] ?>"/>
        ...
        foreach ($shop as $shop) {
            ...
            <tr>
                ...
                <td><a class="btn btn-success btn-sm"
                       href="/<?= $adm ?>/pers/edit/<?= $shop['id'] ?>">Редактировать</a></td>
                <td>
